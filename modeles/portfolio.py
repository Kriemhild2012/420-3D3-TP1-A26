...
class PortfolioSujet(Sujet):
    """Sujet concret: contient l'état du portefeuille et les prix courants."""
    ...
    def get_donnees(self) -> dict:
        """Retourne toutes les données utiles aux observateurs sous forme de dict."""
        titres = {}
        valeur_totale = 0.0
    ...
    def recuperer_prix(ticker):
        ...
        if prix is None or ouverture is None:
            raise ValueError(f"Le titre '{ticker}' n'a pas de prix disponible.")
        return float(prix), float(ouverture)

    def actualiser_prix(self):
        """Récupère les prix, met l'état à jour et notifie les observateurs."""
        for ticker in self.titres:
            prix, ouverture = self.recuperer_prix(ticker)
            self.prix[ticker] = prix
            self.ouvertures[ticker] = ouverture

        self.derniere_mise_a_jour = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.notifier()
    ...
    def ajouter_titre(self, ticker, quantite, seuil_bas=None, seuil_haut=None):
        ...
        if ticker in self.titres:
            raise ValueError(f"{ticker} est déjà dans le portfolio.")
        if quantite <= 0:
        ...
        if seuil_bas <= 0 or seuil_haut <= 0 or seuil_bas >= seuil_haut:
            raise ValueError("Le seuil bas doit être positif et inférieur au seuil haut.")
    ...
    def retirer_titre(self, ticker):
        if ticker not in self.titres:
            raise ValueError(f"{ticker} n'est pas dans le portfolio.")
    ...
    def modifier_titre(self, ticker, quantite=None, seuil_bas=None, seuil_haut=None):
        ...
        if seuil_bas is not None or seuil_haut is not None:
            ...
            if seuil_bas <= 0 or seuil_haut <= 0 or seuil_bas >= seuil_haut:
                raise ValueError("Le seuil bas doit être positif et inférieur au seuil haut.")
            self.titres[ticker]["seuil_bas"] = round(float(seuil_bas), 2)
            self.titres[ticker]["seuil_haut"] = round(float(seuil_haut), 2)
